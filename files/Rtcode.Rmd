Import and install libraries required:
...
Then we set the parameters for analysis. The values for the delay between onset of sx and shedding into wastewater (in days)
Mean & SD of serial interval for Rt estimation (in days) are both from literature review.
...
Now we import the data. Just to be clear, I was using the HK data and the data requirements (and subsequent parameters) are changed a little. I Took the final day of the epidemiological week as sampling date, and converted 7-day geometric mean of GC/L per capita into per mL. That was kind of brief but there's only so much we can do with limited data, innit? Also don't forget to switch it into list, in which viral loads are in the form of numeric vector with NAs removed
...
Then there goes the analysis. Here I also calculated uncertainties so we can adjust for it later. And because Hong Kong reports surveillance data weekly, notice that 'time_step = “week”'.
...
Then there's just the boring stuff where I plotted the findings with 95% confidence intervals shown with the ribbon. Don't ask me why I chose that colour, I want to know too.
